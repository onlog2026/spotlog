...
export function FormularioComercialV3({ compact: _compact = false }: { compact?: boolean }) {
    ...
    >
      ...
      <label className="mt-6 flex items-start gap-3 cursor-pointer group">
        ...
        <span className="text-sm text-ink-600 leading-relaxed">
          ...
          <Link href="/privacidade" className="text-navy-600 font-semibold underline">
...
const inputClass =
  "w-full rounded-xl border border-navy-200 bg-white px-4 py-3 text-navy-900 placeholder:text-ink-400 focus:border-navy-500 focus:outline-none focus:ring-2 focus:ring-navy-500/20 transition-all";
  ...
  return (
    <label className="block">
      <span className="block text-sm font-semibold text-navy-900 mb-1.5">
        {label} {required && <span className="text-navy-500">*</span>}
      </span>
      {children}
    </label>
  );
